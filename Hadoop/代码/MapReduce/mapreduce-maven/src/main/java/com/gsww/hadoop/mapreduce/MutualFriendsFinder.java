 ...
 *      O:A,H,I,
 * 分析：
 *  //Mapper输出：如果k < v，则k-v；否则v-k
 *  A-B,A-C,A-D,A-F,A-E,A-O
 *  A-B,B-C,B-E,B-K
 *  C-F,A-C,C-D,C-I
 *  A-D,D-E,D-F,D-L
 *  ...
...
//        conf.set("mapreduce.framework.name","local");
    ...
    static class MutualFriendsFinderMapper extends Mapper<LongWritable, Text, Text,NullWritable> {
        Text kOut = new Text();
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            for (String friend : friends) {
                if (person.compareTo(friend) < 0) {
                    kOut.set(person + "-" + friend);
                }
                else if (person.compareTo(friend) > 0) {
                    kOut.set(friend + "-" + person);
                }
    ...
    static class MutualFriendsFinderReducer extends Reducer<Text,NullWritable,Text, NullWritable> {
        @Override
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException, URISyntaxException {
        ...
        job.setMapperClass(MutualFriendsFinderMapper.class);
        job.setReducerClass(MutualFriendsFinderReducer.class);
