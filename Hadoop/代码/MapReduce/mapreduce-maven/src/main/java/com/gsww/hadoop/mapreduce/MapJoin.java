...
import com.gsww.hadoop.mapreduce.mrbean.ProductBean;
...
//        conf.set("mapreduce.app-submission.cross-platform","true");
    ...
    static class MapJoinMapper extends Mapper<LongWritable, Text, Text, NullWritable> {
        ...
        //存储从DistributedCache加载到Map Task本地的文件内容的数据结构
        Map<String,ProductBean> productMap = new HashMap<>();

        Text kOut = new Text();
        /**
         * setup方法是在Map Task处理数据时仅调用一次，可以做一些初始化工作。
         * @param context
         * @throws IOException
        ...
        protected void setup(Context context) throws IOException, InterruptedException {
            ...
            /**
             * 文件所在位置就是该Map Task程序运行的工作目录，即当前目录
             */
//            BufferedReader br = new BufferedReader(new FileReader("t_product.txt"));
            BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream("t_product.txt")));//防止中文乱码
            String line = null;
            while ((line = br.readLine()) != null) {
                //对每一个产品都要新建product bean
                ProductBean productBean = new ProductBean();

                String[] fields = line.split("\t");
                productBean.setProductBean(fields[0],fields[1],Float.parseFloat(fields[3]),fields[2]);

                //加载产品信息表到内存数据结构
                productMap.put(fields[0],productBean);
            }
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            //从内存数据结构查询该订单的相关产品信息，并为map方法的输出KEYOUT设置
            kOut.set(orderLine + '\t' + productMap.get(fields[2]));
            //输出到结果文件
